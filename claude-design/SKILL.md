...
metadata:
  hermes:
    ...
    related_skills: [design-md, popular-web-designs, excalidraw, architecture-diagram, sketch, double-evolution]
---
...
**Before starting, check for other web-design skills like `design-md` (71-brand design token reference library: Apple, Stripe, Linear, Vercel, Airbnb, etc.) and `popular-web-designs` (ready-to-paste design systems for Stripe, Linear, Vercel, Notion, etc.).** If the user wants a known brand's look, load `design-md` alongside this one and read the brand's DESIGN.md for its visual vocabulary. If the deliverable is a token spec file rather than a rendered artifact, use `design-md` instead. Full decision table below.
...
| **design-md** | 71-brand design token reference library (Apple, Stripe, Linear, Vercel, Airbnb, Notion, etc.) — load alongside claude-design when the user wants a specific brand's visual vocabulary. Also supports Google's DESIGN.md spec format for authoring/validating/diffing design-token files. | a specific brand's design tokens applied to an artifact, or a formal DESIGN.md token spec file |
...
These compose: use `design-md` for brand-specific design tokens, `popular-web-designs` for ready-to-paste CSS systems, and `claude-design` for how to turn a brief into a thoughtful local HTML file.
...
For Chinese promotional event posters (活动海报, 嘉年华, 促销海报) aimed at social media sharing, load `references/chinese-promo-poster-patterns.md` — it has the canonical 1080×1920 layout structure, 5-style variant palette, and genre-specific techniques.
