...
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| Model Serving | 用户有推理服务代码需要边界检查 | "这个inference server的边界条件都覆盖了吗" |
| Agent系统 | 用户有Multi-Agent编排代码 | "Agent工作流的edge case穷举一下" |
| Prompt处理 | 用户有prompt构建/解析代码 | "prompt template的边界条件" |
| Token处理 | 用户有token计数/截断逻辑 | "token truncation的边界穷举" |
| RAG Pipeline | 用户有检索增强生成管线 | "RAG pipeline的输入边界全吗" |

#### 产品 / 商业
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| 支付流程 | 用户有支付/交易代码 | "支付流程的边界条件都覆盖了吗" |
| 用户注册 | 用户有注册/登录代码 | "注册流程的edge case穷举" |
| 权限系统 | 用户有RBAC/ACL代码 | "权限检查有没有遗漏的分支" |
| 订阅管理 | 用户有订阅/计费代码 | "订阅状态的边界条件检查" |
| 通知系统 | 用户有通知/推送代码 | "通知触发的所有分支覆盖了没" |

#### 安全 / 合规
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| 认证鉴权 | 用户有OAuth/JWT代码 | "auth流程所有边界条件覆盖了吗" |
| 输入校验 | 用户有用户输入处理代码 | "输入校验的边界条件穷举" |
| SQL注入 | 用户有数据库查询代码 | "所有查询路径的SQL注入边界" |
| XSS防御 | 用户有HTML渲染代码 | "渲染路径的XSS边界覆盖" |
| 数据脱敏 | 用户有数据脱敏/加密代码 | "脱敏逻辑的边界条件" |

#### 技术 / 架构
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| API接口 | 用户有REST/GraphQL接口 | "这个API所有参数的边界覆盖" |
| 数据库操作 | 用户有CRUD/迁移代码 | "数据库操作的边界条件" |
| 消息队列 | 用户有MQ/事件处理代码 | "消息处理的边界和重试逻辑" |
| 微服务调用 | 用户有服务间调用代码 | "服务调用的超时/降级边界" |
| 并发/锁 | 用户有并发控制代码 | "并发路径的race condition边界" |

#### 前端 / 交互
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| 表单处理 | 用户有表单校验代码 | "表单所有输入组合都覆盖了吗" |
| 状态管理 | 用户有状态机/Reducer代码 | "状态机的所有转移都覆盖了没" |
| 路由处理 | 用户有路由守卫/导航代码 | "路由跳转的所有边界" |
| 文件上传 | 用户有文件处理代码 | "上传的所有文件格式边界" |
| 动画/过渡 | 用户有动画/transition代码 | "动画状态的所有边界" |

#### 数据 / ETL
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| CSV/JSON解析 | 用户有数据解析代码 | "文件解析的所有格式边界" |
| 数据迁移 | 用户有数据迁移脚本 | "迁移脚本的边界条件" |
| 聚合计算 | 用户有统计/聚合代码 | "聚合逻辑的除零/null边界" |
| 数据校验 | 用户有数据质量检查代码 | "校验规则的所有边界" |
| 流处理 | 用户有流式处理代码 | "流处理的窗口边界和延迟" |

#### DevOps / 基础设施
| 场景 | 触发信号 | 示例 |
|------|---------|------|
| CI/CD脚本 | 用户有构建/部署脚本 | "部署脚本的所有失败分支" |
| 配置管理 | 用户有配置文件解析代码 | "配置缺失/错误的边界" |
| 容器编排 | 用户有Docker/k8s配置 | "容器启动失败的所有边界" |
| 健康检查 | 用户有health check代码 | "健康检查的超时和重试边界" |
| 日志/监控 | 用户有日志采集代码 | "日志轮转/磁盘满的边界" |
