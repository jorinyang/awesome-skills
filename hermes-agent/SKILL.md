...
**Provider capability caveat — especially MiniMax:**

Not all providers support all auxiliary tasks. MiniMax is a common trap: it has `image-01` / `image-01-live` which are **image generation** models (text → image), but it has **no vision-understanding model** for `auxiliary.vision`. Configuring `auxiliary.vision.provider = minimax-cn` will silently fail — the API responds "no image provided" even with correct base64 input, because the endpoint does not exist on that provider.

Workaround: use a provider with an actual vision model (OpenAI GPT-4o, Google Gemini, OpenRouter-routed models with vision, etc.) for `auxiliary.vision`, even if the main model still runs on MiniMax-CN.

> Full error transcript, curl reproducer, and verified model table: `references/minimax-vision-failure.md`
...
- `references/skill-authoring.md` — Authoring in-repo SKILL.md files: frontmatter, validator, structure, community skill adaptation, methodology absorption
- `references/s6-container-supervision.md` — Docker s6-overlay supervision tree: adding services, debugging profile gateways, Architecture B pattern
- `references/adapting-community-skills.md` — Path mappings, tool mappings, runtime-neutrality checklist for adapting community skills to Hermes
- `references/methodology-absorption.md` — Framework for systematically evaluating and absorbing external methodologies into Hermes skills ecosystem
