...
Script path: auto-discovered — use `search_files` to find `maps_client.py` if the default below doesn't resolve on your profile.

## Commands

```bash
MAPS=~/.hermes-feishu/skills/productivity/maps/scripts/maps_client.py
```
...
- **OSRM routing coverage is best for Europe and North America. In China, walking/cycling/transit data is absent — use Amap MCP instead (see references/amap-mcp.md).**
...
- **China POI data is extremely sparse: `nearby` returns 0 results for most Chinese city queries even at 500m radius. Use Amap MCP for any POI or transit task in China.**

## Verification

```bash
python3 ~/.hermes-feishu/skills/productivity/maps/scripts/maps_client.py search "Statue of Liberty"
# Should return lat ~40.689, lon ~-74.044

python3 ~/.hermes-feishu/skills/productivity/maps/scripts/maps_client.py nearby --near "Times Square" --category restaurant --limit 3
# Should return a list of restaurants within ~500m of Times Square
```

## China Coverage (高德地图 MCP)

The OSM/OSRM-based maps skill has severe limitations in China:
- POI search returns 0 results for county-level cities (兴义, 安龙)
- Walking/cycling modes degrade to driving data (all modes return same distance)
- No public transit routing at all

For ANY China maps task, prefer the **Amap MCP server** (`amap-mcp-server`) instead.
See `references/amap-mcp.md` for setup and usage.

For weather in China (7-day/15-day forecasts, hourly data, alerts), use the **weather MCP**
(Alibaba Cloud API Gateway). See `references/weather-mcp.md` for config and tool reference.
Critical for trip planning: returns 雷电/暴雨/大风 real-time alerts.
