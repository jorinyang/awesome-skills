...
**反向需求追溯**：当用户不是要求产出分析文档，而是查询某个具体功能/字段的需求全貌时（如"XXX字段的需求是什么"），使用 [references/requirement-tracing-reverse-lookup.md](references/requirement-tracing-reverse-lookup.md) 中的反向查找工作流。两者共享源材料读取方法，但输出形式不同。
...
| 触发条件 | 症状 | 修复方案 | 降级方案 |
|---------|------|---------|---------|
| docx 节点实为上传文件 | `3380002: Unsupported document type 'file'` | 改走 `wiki +node-get` → `drive +download` 下载后读取 | 请用户手动复制文档内容为飞书原生文档 |
| wiki 下载缺 scope | `wiki +node-get` 报 `missing_scope` | `lark-cli auth login --scope "wiki:node:retrieve"` 授权 | 请用户直接在对话中粘贴源材料文本 |
| PDF 文本提取失败 | PyPDF2 报错或输出乱码/空白 | 尝试 `pdftotext` 或 `marker-pdf` 备用工具 | 标注"⚠️ PDF解析失败"，请用户提供文本版 |
| 源材料内容为空 | 读取成功但无有效文本输出 | 检查文件是否损坏，尝试重新上传/导出 | 跳过该材料，在报告中标注"📭 待补充" |
| 缺少模块范围定义 | 源材料中无明确模块列表 | 从会议纪要和PRD标题中推导模块边界 | 使用通用模块模板，标注"⚠️ 模块待确认" |
| 多轮对齐缺少日期 | 源材料中无会议日期信息 | 从文件名/飞书文档元数据中提取 | 标注为"日期待补充"，不阻塞分析 |
...
1. 并行读取所有源材料（PRD + 所有轮次的录音转写 + 会议纪要）
   ...
   🔍 **CHECKPOINT D4-1：源材料读取完成。** 确认：(a) 每个源材料已成功读取并包含有效文本内容；(b) 状态标注图例（🟢🟡🔵🔴）已就绪。如任一材料读取失败或为空，查看下方「失败模式与降级策略」选择对应降级路径，不得继续进入交叉对比。
   ...
   🔍 **CHECKPOINT D4-2：交叉对比与标注完成。** 确认：(a) 所有模块的需求项已逐项追踪 PRD→第一次对齐→第二次对齐 完整链条；(b) 每个需求项状态（原有/差异/新增/待确定）已正确标注；(c) 所有差异项包含完整的三段式说明 原设计→变更后→变更原因；(d) 待确定事项已汇总。通过后方可产出飞书文档。
