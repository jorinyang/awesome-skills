...
**此策略仅适用于康养长期团（单基地多日活动）**。对于私人定制游览团（多日多点打卡），住宿应跟随行程地理分配，而非预设单一基地——减少换酒店次数但优先匹配当日动线：

- 原则：同一方向景区尽量安排同一住宿点，换酒店 ≤ 2 次（含首晚）
- 示例：万峰林日 → 住万峰林民宿；峡谷+市区日 → 住市区酒店
- 避免：连住万峰林 3 晚但 D2 跑马岭河（东北方向往返 1h+）

询问住宿时，先确认行程骨架再看地理分配，不在 Clarify 阶段预设「全部住 XX」。
...
当用户给出已有方案 + 变换指令（删减天数、修改某天、压缩、注入新元素）时，参考 [`variant-generation.md`](variant-generation.md) 的标准化操作。

常见变体：
- **COMPRESS**: 14天→7天（保留所有疗愈，合并同类晚间项目，删除远程景点）
- **MODIFY**: Day6 注入水上+探洞一日模式（桨板+竹筏→岩架鱼→犀牛洞）
- **INJECT**: 完善餐饮方案（退休职工友好：少油少辣、清蒸清炖、每餐详细标注）

## 与下游管线衔接

本阶段产出物为飞书 docx 行程方案。后续进入 `travel-workflow` 管线时，需将行程方案结构化抽取为 `trip.json`，再由下游脚本生成交付物。

> **定制方案（研学/夏令营）** 使用不同的工作流，不走 trip.json 管线。详见 `references/study-tour-design.md`。
