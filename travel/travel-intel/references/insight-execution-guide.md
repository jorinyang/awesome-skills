...
**咨询洞察节点已超过 750 篇文档，`--page-all` 默认 `--page-limit 10`（500 条静默截断）会导致 06-10 之后的每日简报和采集文档完全不可见，weekly cron 找不到本周数据直接失败（Broken pipe）。**

| page-limit | 可见范围 | 状态 |
|:--:|------|:--:|
| 10（默认） | 500 条 | ❌ 漏掉 250+ 条 |
| 15 | 750 条 | ⚠️ 可见到 06-11，可能不够 |
| 20 | 1000 条 | ✅ **推荐**（当前节点规模） |
| 0（不限） | 全量 | ⚠️ 大节点可能超时（60s+） |

**所有 `wiki +node-list` 命令必须显式加 `--page-limit 20`**。`--page-limit 0` 在 750+ 条节点上实测超时，20 是当前最佳平衡点。
...
| **page-limit 截断** | **漏掉最新文档 → Broken pipe** | **所有 node-list 加 --page-limit 20** |
...
for tok in V0Lhwl7KYiWYDDk1vCncv2GhnYf EAMYw1CPoipVWtkObbtcR2oDnNc UF7Cw5w2WiHGfjkKVvBcxj8Hnib; do
  ...
  lark-cli wiki +node-list --space-id 7643710721485753535 \
    --parent-node-token $tok --page-all --page-limit 20 --as bot 2>&1 \
    | grep "{YYYY}_WW周_周度分析"
done
```
...
lark-cli wiki +node-list --space-id 7643710721485753535 \
  --parent-node-token V0Lhwl7KYiWYDDk1vCncv2GhnYf --page-all --page-limit 20 --as bot

# 竞品动态（子分类 — 含6月4日前的历史文档）
lark-cli wiki +node-list --space-id 7643710721485753535 \
  --parent-node-token EAMYw1CPoipVWtkObbtcR2oDnNc --page-all --page-limit 20 --as bot

# ★ 咨询洞察一级分类（含6月5日起的新文档 — 每日简报/综合洞察都可能在此）
lark-cli wiki +node-list --space-id 7643710721485753535 \
  --parent-node-token UF7Cw5w2WiHGfjkKVvBcxj8Hnib --page-all --page-limit 20 --as bot
```
