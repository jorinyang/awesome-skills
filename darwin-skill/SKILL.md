...
- 🔴 **硬限制：每夜最多优化 2 技能，最多 1 个 delegate_task 子 agent**（cron idle timeout 600s；子 agent 等待时间计入父 agent idle）
- 每个子 agent ≤2 技能
...
- **Git 可用性 + 技能数前置检查**：如 git 可用且待优化技能 ≥3 个，自动降级为 Phase 1 only（评分报告），避免进入 Phase 2 超 idle timeout
