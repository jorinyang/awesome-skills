...
> 参考 `ai-native-workflow` Phase ① 澄清 + grill-me 方法论

当用户意图模糊或缺少关键决策信息时，先追问核心问题再进入内容理解：

- 这个页面的受众是谁？他们看完要做什么？
- 核心信息优先级？（Top 3 必须传达的事）
- 有风格参考吗？有无品牌规范？
- 产出是内部用还是对外展示？

**跳过条件**：用户指令已明确（如"用昨天那个版本做个 web"），直接进入阶段一。
...
#### 阶段一 B：设计决策澄清（grill-me 方法论）

**目的**：在进入设计阶段前，用 grill-me 方法论遍历页面设计的关键决策树，确保设计方向有共识后再动手。

**方法**：
1. 根据页面类型选择 3-4 个最相关的追问维度
2. 逐维度追问，提供推荐答案
3. 将决策记录到阶段二的规划中

**追问维度**：

| 追问维度 | 展示性页面 | 功能性页面 | 混合页面 |
|---------|-----------|-----------|---------|
| **目标受众** | 谁看？一次性浏览还是反复查阅？ | 谁用？频率？技术水平？ | 两者都问 |
| **核心行动** | 受众看完要带走什么认知？（≤3条） | 用户完成什么任务算成功？ | 两者都问 |
| **风格方向** | 有参考吗？（提供链接）不要什么风格？ | UI 模式参考？（Notion/Linear/飞书） | 展示+功能各自参考 |
| **信息优先级** | 哪 3 件事必须首屏可见？ | 哪个操作是最高频的？ | 两者都问 |
| **设备场景** | 主要在桌面还是手机浏览？ | 桌面操作还是移动操作？ | 两者都问 |
| **反目标** | 明确不要什么？（花哨/信息过载） | 明确不要什么？（多步流程/复杂表单） | 两者都问 |

**跳过条件**：
- 用户已明确指定了风格/受众/目标 → 跳过已明确的维度，只追问未明确的
- 贵州之客常规页面 → 使用品牌默认风格，可跳过风格方向追问
- 用户说"直接做" → 跳过此步骤，用默认假设进入阶段二
...
#### 设计质量审查（design-review 方法论）

以上为技术性校验（链接/按钮/渲染/兼容性），以下是设计质量维度的审查——对照设计决策（阶段一 B）检查实现质量：

> 🆕 **增强审查层**：本阶段可调用以下独立技能作为质量门禁：
> - `editorial-review-structure` — 使用5种结构模型审查页面TAB结构和信息层级
> - `edge-case-hunter` — 穷举所有交互路径的边界条件（空状态/加载中/错误/权限不足）

| 审查维度 | 检查项 | 通过标准 |
|---------|-------|---------|
| **视觉层次** | 最重要的内容是否最突出？字号层级是否形成清晰阶梯？ | 标题→副标题→正文→说明，层次分明 |
| **一致性** | 间距/色彩/圆角/阴影是否从共享设计系统中取值？同类组件是否视觉一致？ | 无硬编码值飘移，同类元素外观一致 |
| **美学贯彻** | 是否贯彻了阶段一 B 中确认的风格方向？有无突兀元素？ | 整体观感符合风格方向，无跳出元素 |
| **组件质量** | 新组件是否与已有组件共存不冲突？有无重复组件应合并？ | 无重复实现，命名遵循项目约定 |
| **状态覆盖** | 按钮/链接/卡片是否有 hover/active/focus 态？表单是否有 error/success/disabled 态？ | 所有交互元素有完整状态 |
| **响应式质量** | 不只是缩小，而是重新组织（如桌面双列→移动单列） | 三断点均可用，触控区域 ≥44×44px |
| **暗色模式** | 如项目支持暗色模式，CSS 变量是否全覆盖？暗色调是否刻意设计而非简单反色？ | 暗色下对比度充足，视觉舒适 |

**审查方法**：
- 部署后用 Playwright 截取三断点截图（1280/768/375）
- 对照截图逐项检查上述维度
- 发现问题分级：Must Fix（功能受阻）/ Should Fix（有损体验）/ Could Improve（锦上添花）
