...
### 快速评估模式（Quick Assessment）

当用户的问题是"这个仓库怎么样"、"是否适用于X"、"能不能用于Y场景"等**评估性问题**，而非"吸收这个仓库"、"创建技能"等**执行性指令**时，使用快速评估模式：

| 用户意图 | 模式 | 执行阶段 |
|---------|:---:|---------|
| "评估一下这个仓库" | 快速 | Phase 1-3 → 输出评估报告 |
| "是否适用于X项目" | 快速 | Phase 1-3 → 针对X场景分析 |
| "帮我看看这个项目" | 快速 | Phase 1-3 → 输出评估报告 |
| "吸收这个仓库" | 完整 | Phase 1-8 全流程 |
| "创建技能" | 完整 | Phase 1-8 全流程 |
| "能不能用" → 用户确认后 | 完整 | Phase 4-8 续接 |
...
**核心定位**：一句话说明仓库是什么
**业务域评分**：AI={X} 数字化={X} 咨询={X} 企业管理={X} FDE={X}
**适用场景**：✅ 适合 / ❌ 不适合 / ⚠️ 有限适用
**关键发现**：2-3 条核心洞察
**建议**：是否值得深入评估或直接吸收
```

快速评估仍需完成 Phase 1-3 的全部分析步骤（URL解析、README深度阅读、业务域评分），只是跳过 Phase 4-8 的执行环节。如果快速评估结果为正面（max评分≥3），主动询问用户是否需要进入完整吸收流程。
