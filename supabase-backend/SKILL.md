...
| 28 | **`CREATE POLICY IF NOT EXISTS` 语法错误 + INSERT 策略用法** | ① `CREATE POLICY IF NOT EXISTS ...` → `syntax error at or near "NOT"` ② `CREATE POLICY ... FOR INSERT USING (true)` → `only WITH CHECK expression allowed for INSERT` ③ 第一条策略失败导致整个事务 abort → 后续命令被静默忽略 | ① Supabase PG 版本不支持 `IF NOT EXISTS` 语法 ② INSERT/UPDATE 策略的 USING 子句需独立声明，INSERT 只能 `WITH CHECK`，UPDATE 需 `USING ... WITH CHECK ...` ③ psycopg2 单次 execute 内多条语句共享同一事务 | **安全模式**：先 `DROP POLICY IF EXISTS "name" ON table`（失败也不抛异常），再 `CREATE POLICY "name" ON table FOR <op> USING (true) WITH CHECK (true)`。**INSERT 策略**只用 `CREATE POLICY ... FOR INSERT WITH CHECK (true)`，不带 USING。**每条策略用单独的 execute**，避免某条失败导致事务 abort 吞掉后续命令。**建策略前先确认表已存在**（`DROP POLICY IF EXISTS` 在表不存在时也会报 `UndefinedTable`） |
